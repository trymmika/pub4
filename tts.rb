...
# SETUP INSTRUCTIONS FOR WEIRD/LOFI PIPER VOICES
setup:
  install_piper:

    command: pip install piper-tts

  download_weird_voices:
    # Low-quality voices (grainy, artifacts)

    - piper --download en_US-lessac-low

    - piper --download en_GB-northern_english_male-low

    - piper --download en_US-amy-low

    - piper --download en_US-libritts-high  # Robotic despite "high"

  download_experimental:
    # Community weird voices

    hal9000:

      source: https://github.com/dividebysandwich/piper-voice-models

      files: [hal9000-no-denoise.onnx, hal9000-no-denoise.onnx.json]

    glados:

      source: https://github.com/dividebysandwich/piper-voice-models

      files: [glados.onnx, glados.onnx.json]

    retro_synths:

      source: https://eurpod.com/piper-voices-of-retro-speech-synthesizers/

      voices: [keynote_gold, nvda_espeak]

  verify:
    command: piper --list-voices
...
engine:
  ...
  piper_models:
    # LOFI/WEIRD VOICES (intentionally grainy, artifacts, mechanical)

    weird_lofi:

      - en_US-lessac-low          # 16kHz, grainy, compressed artifacts

      - en_GB-northern_english_male-low  # Robotic, mechanical quality

      - en_US-amy-low             # Compressed, artifact-rich

      - en_US-libritts-high       # Robotic prosody (despite "high")

    # EXPERIMENTAL (download from community repos)
    experimental:

      - hal9000-no-denoise        # github.com/dividebysandwich - cinematic, noisy

      - glados                    # Portal robot - synthetic

      - wheatley                  # Portal comic robot

      - keynote_gold              # eurpod.com - retro synth, grainy

      - nvda_espeak               # eurpod.com - old screen reader

    # CLEAN VOICES (boring, too natural)
    clean:

      - en_US-lessac-medium       # 22kHz balanced (not using this)

      - en_US-lessac-high         # 22kHz most natural (boring)
...
effects:
  ...
  # 5-15 SOLUTIONS FOR COMFY + EFFECTS (RESEARCH-BACKED):
  principles:

    1_preserve_mids: "Boost 1-3kHz (speech critical) for intelligibility"
    ...
    6_compression: "Light multiband compression keeps volume consistent"

    7_eq_presence_boost: "2-4kHz boost = clarity without harshness"

    8_gentle_reverb: "Short plate/room reverb = less dry, still clear"

    9_saturation: "Tube/tape saturation = warmth + harmonics"

    10_deessing: "Tame harsh sibilants (6-8kHz) for comfort"

    11_noise_gate: "Remove artifacts between words for cleaner output"

    12_stereo_widening: "Haas effect/chorus for immersive spatial feel"

    13_formant_preservation: "Pitch shift with formant lock = natural"

    14_dynamic_effects: "Vary effect intensity per phrase for interest"

    15_postprocessing_chain: "Normalize → EQ → Compress → Effects → Limit"
...
execution:
  ...
  continuous_mode:
    ...
    recommended_solution: |
      # PowerShell Background Job with Piper TTS (TESTED & WORKING)

      $job = Start-Job -ScriptBlock {

        $tips = @(

          "Did you know breathing is actually quite important? Most people forget.",

          "Pro tip: Try opening your eyes when you wake up. Game changer.",

          "Here's a life hack: doors work better when you turn the handle.",

          "Advanced technique: water is more hydrating when you actually drink it.",

          "Fun fact: stairs are easier to climb when you lift your feet.",

          "Life pro tip: food tastes better when you actually put it in your mouth.",

          "Did you realize sleep works better with your eyes closed? Revolutionary.",

          "Here's wisdom: standing is more effective when you're not sitting."
        ...
        while($true) {
          ...
          try {
            ...
            $advice = $tips | Get-Random

            echo $advice | piper -m en_US-lessac-medium --output_file voice.wav 2>$null

            powershell -c "(New-Object Media.SoundPlayer 'voice.wav').PlaySync()" 2>$null
      ...
      $job = Start-Job -ScriptBlock {
        ...
        while($true) {
          ...
          $advice = @("Breathe", "Open eyes", "Turn handle") | Get-Random

          echo $advice | piper -m en_US-lessac-medium --output_file temp.wav 2>$null

          ffmpeg -i temp.wav -af "acrusher=bits=8:mix=0.8,lowpass=5000" out.wav 2>$null

          powershell -c "(New-Object Media.SoundPlayer 'out.wav').PlaySync()"

          Remove-Item temp.wav, out.wav
