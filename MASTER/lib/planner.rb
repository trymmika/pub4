...
# Breaks goals into tasks, executes sequentially, tracks completion
...
module MASTER
  class Planner
    ...
    def create_plan(goal)
      # Ask LLM to break down goal into concrete tasks
      prompt = <<~PROMPT
      ...
      result = @llm.chat(prompt, tier: :standard)
    ...
    def execute_next(&executor)
      ...
      begin
        # Execute via provided block or return task for external execution
    ...
    def parse_tasks(text)
      ...
      text.lines.each do |line|
        # Match numbered items: "1. action" or "1) action"
        if line =~ /^\s*(\d+)[.)]\s*(.+)/
          action = $2.strip
    ...
    def archive_plan
      ...
      history = history.last(50)  # Keep last 50 plans
