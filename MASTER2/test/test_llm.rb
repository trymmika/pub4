...
class TestLLM < Minitest::Test
  ...
  def test_rates_defined
    ...
    assert MASTER::LLM::RATES.key?("deepseek/deepseek-v3")
    assert MASTER::LLM::RATES.key?("openai/gpt-4.1-mini")
    assert MASTER::LLM::RATES.key?("openai/gpt-4.1-nano")
  end
  ...
  def test_circuit_available_when_no_failures
    ...
    assert MASTER::LLM.circuit_available?("deepseek/deepseek-r1")
  end
  ...
  def test_record_cost
    MASTER::LLM.record_cost(model: "deepseek/deepseek-r1", tokens_in: 1000, tokens_out: 500)
